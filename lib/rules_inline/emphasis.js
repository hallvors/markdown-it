...
// find next delimiter; if it's a closing delimiter, calculate which opening
// one it corresponds to
function calculateNextPair(state, cache) {
  var ch, currDelim, lastDelim, delimiters, i, j, oldPos, scanned,
      max = state.posMax;
...
// get a closing delimiter position
function getMatchingPair(state, start) {
  ...
  while (!delimiters.length || (delimiters[0].end < 0 && delimiters[0].open)) {
